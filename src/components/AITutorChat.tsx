...
export default function AITutorChat() {
  ...
  return (
    <div id="ai-tutor-chat" className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm rounded-2xl overflow-hidden flex flex-col h-[520px] transition-colors duration-300 font-sans">
      <div className="bg-slate-50 dark:bg-slate-950 border-b border-slate-200 dark:border-slate-800 p-4 flex justify-between items-center shrink-0">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <span className="text-xs text-slate-700 dark:text-slate-400 font-medium font-sans">Resolución de dudas en tiempo real</span>
          </div>
        </div>
      ...
      {warningMsg && (
        <div className="bg-amber-50 dark:bg-amber-950/20 border-b border-amber-100 dark:border-amber-900/40 p-2 text-center text-xs text-amber-700 dark:text-amber-400 font-medium">
          {warningMsg}
        </div>
      )}
      ...
      <div className="flex-1 p-4 overflow-y-auto space-y-4 bg-slate-50/50 dark:bg-slate-950/10">
        {messages.map((m) => (
          <div key={m.id} className={`flex gap-3 max-w-[85%] ${m.sender === "user" ? "ml-auto flex-row-reverse" : ""}`}>
            ...
            <div className={`p-3.5 rounded-2xl text-xs sm:text-sm leading-relaxed ${m.sender === "user" ? "bg-indigo-600 text-white rounded-tr-none whitespace-pre-wrap" : "bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-200 rounded-tl-none shadow-sm"}`}>
              {m.sender === "user" ? m.text : <MathMarkdownRenderer content={m.text} />}
              <span className={`block text-xs mt-1.5 text-right ${m.sender === "user" ? "text-indigo-200" : "text-slate-500 dark:text-slate-400 font-sans"}`}>
                {m.timestamp}
              </span>
            </div>
          </div>
