...
export default function MathEquation({ title, htmlLayout, termExplanations }: MathEquationProps) {
  ...
  return (
    <div className="bg-slate-50 dark:bg-slate-900/40 border border-slate-200 dark:border-slate-800 rounded-xl p-5 hover:border-slate-300 dark:hover:border-slate-700 transition-colors duration-200 shrink-0 font-sans">
      {title && (
        <h4 className="text-xs font-semibold text-slate-600 dark:text-slate-400 uppercase tracking-wider mb-3 font-mono">
          {title}
        </h4>
      )}
      
      {/* Mathematical display area */}
      <div className="flex flex-col items-center justify-center py-6 px-4 bg-slate-100/60 dark:bg-slate-950/80 border border-slate-200/80 dark:border-slate-800/60 rounded-lg shadow-inner-sm overflow-x-auto">
      ...
      <div className="mt-4">
        <p className="text-xs text-slate-600 dark:text-slate-400 mb-2 font-medium">Fórmula interactiva: toca o pasa el cursor sobre los términos para explorar su lógica</p>
